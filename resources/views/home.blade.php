        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class= "row">
                        <div class="col-4">
                            <img src="{{ $p->preview_pic }}" class="img-thumbnail preview_pic" data-playlist="{{ $p->id }}">
                        </div>
                        <div class= "col-8 ">
                            <span class="album-name" data-playlist="{{ $p->id }}">{{ $p->name }}</span>
                            <img src="{{ asset('/img/modify.png') }}" class="little_img ren-pl" data-playlist="{{ $p->id }}">
                            <img src="{{ asset('/img/bin.png') }}" class="little_img del-pl" data-playlist="{{ $p->id }}">
                            
                            
                        </div>
                                <div class="hidden px-3 py-2 playlist-songs" data-playlist="{{ $p->id }}">
                                    @if(count($p->songs)>0)
                                    @foreach($p->songs as $song)
                                    <div class="song_card card" data-playlist="{{ $p->id }}" data-song="{{ $song->id }}" data-img="{{ $song->img }}" data-title="{{ $song->title }}" data-album="{{ $song->album }}" data-artists="{{ $song->artists }}" data-length="{{ $song->length }}">
